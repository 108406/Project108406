...
var listWithProject = async function (project_id) {
    //存放結果
    var result = [];

    await query("select project_id, list_name from projectlist_view where project_id = '" + project_id + "'")
        .then((data) => {
            if (data.rowCount > 0) {
                result = data; //成功
            } else {
...
//匯入
module.exports = { projectWithUser, workWithUser, listWithProject }